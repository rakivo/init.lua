  ...
  config = function()
    ...
    blink.setup({
      ...
      keymap = {
        ...
        -- <C‑n>  — open menu if hidden, otherwise go ↓
        ["<c-n>"] = {
          function(cmp)
            cmp.show()
            cmp.select_next({ on_ghost_text = true, auto_insert = false })
          end,
        },

        -- <c‑p>  — open menu if hidden, otherwise go ↑
        ["<c-p>"] = {
          function(cmp)
            cmp.show()
            cmp.select_prev({ on_ghost_text = true, auto_insert = false })
          end,
        },
        ["<M-Tab>"] = { "accept" },
        ["<Esc>"] = { "hide", "fallback" },
